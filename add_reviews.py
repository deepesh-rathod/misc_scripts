...
for p_id in trial_place_ids:
    ...
    new_reviews = json.dumps(new_review[0:6])
    new_reviews = new_reviews.replace("'","''")

    update_q = f"update gmb_website_details set testimonials ='{new_reviews}' where place_id='{place_id}'"
    sql_query(update_q)
    conn.commit()
